...
# --- SETUP CORRECTLY HERE ---
REFERENCE_RING_DIR = "/data1/zyin/ring_sdf_files" #change to match
    ...
    try:
        ...
        for ring_set in aromatic_rings:
            ...
            if ring_size == 6:
                ...
                if n_count == 0:
                    ...
                    if diff == 1 or diff == 5:  # Adjacent nitrogens
                        aromatic_rings_info.append(('Pyrazine', ring_list))
                    elif diff == 2 or diff == 4:  # Positions 1,3
                        aromatic_rings_info.append(('Pyrimidine', ring_list))
                    else:  # diff == 3, positions 1,4
                        aromatic_rings_info.append(('Pyridazine', ring_list))
                elif n_count >= 3:
                ...
                # Check for Imidazole: 5-membered ring with 2 nitrogens at non-adjacent positions
                if n_count == 2 and 'C' in elements:
                    n_positions = [i for i, elem in enumerate(elements) if elem == 'N']
                    if len(n_positions) >= 2:
                        pos_diff = abs(n_positions[0] - n_positions[1])
                        if pos_diff in [2, 3]:  # Non-adjacent positions in 5-membered ring
                            aromatic_rings_info.append(('Imidazole', ring_list))
                        elif pos_diff in [1, 4]:  # Adjacent positions
                            aromatic_rings_info.append(('Pyrazole', ring_list))
                # Check for rings with N and O (Oxazole or Isoxazole)
                elif n_count == 1 and elements.count('O') == 1 and 'C' in elements:
                    # Determine the positions of N and O to distinguish oxazole vs isoxazole
                    n_pos = elements.index('N')
                    o_pos = elements.index('O')
                    pos_diff = abs(n_pos - o_pos)
                    if pos_diff in [1, 4]:  # Adjacent N and O
                        aromatic_rings_info.append(('Oxazole', ring_list))
                    else:  # Non-adjacent N and O (positions 1,3 or 1,4 depending on counting)
                        aromatic_rings_info.append(('Isoxazole', ring_list))
                # Check for rings with N and S (Thiazole)
                elif n_count == 1 and elements.count('S') == 1 and 'C' in elements:
                    aromatic_rings_info.append(('Thiazole', ring_list))
                elif not heteroatoms:
                    aromatic_rings_info.append(('Cyclopentadiene-like', ring_list))
                elif 'N' in heteroatoms and 'O' not in heteroatoms and 'S' not in heteroatoms and n_count == 1:
                    aromatic_rings_info.append(('Pyrrole', ring_list))
                elif 'O' in heteroatoms and n_count == 0:
                    aromatic_rings_info.append(('Furan', ring_list))
                elif 'S' in heteroatoms and n_count == 0:
                    aromatic_rings_info.append(('Thiophene', ring_list))
                else:
        ...
        for sys in fused_systems:
            if len(sys) == 2:
                ...
                if size_a == 6 and size_b == 6:
                    ...
                    n_count_5 = five_elements.count('N')
                    o_count_5 = five_elements.count('O')
                    s_count_5 = five_elements.count('S')
                    
                    if n_count_5 == 2:  # Imidazole-like
                        bicyclic_fused_info.append(('Benzimidazole-like', list(five_ring), list(six_ring)))
                    elif n_count_5 == 1 and o_count_5 == 1:  # Oxazole/Thiazole-like
                        if 'O' in five_elements:
                            bicyclic_fused_info.append(('Benzoxazole-like', list(five_ring), list(six_ring)))
                        elif 'S' in five_elements:
                            bicyclic_fused_info.append(('Benzothiazole-like', list(five_ring), list(six_ring)))
                    elif 'N' in five_elements:
                        bicyclic_fused_info.append(('Indole-like', list(five_ring), list(six_ring)))
                    elif 'O' in five_elements:
        ...
        if reference_ring_dir and os.path.exists(reference_ring_dir):
            reference_mapping = {
                ...
                'pyridazine.sdf': 'Pyridazine',
                'pyrazine.sdf': 'Pyrazine',
                'pyrrole.sdf': 'Pyrrole',
                'thiophene.sdf': 'Thiophene',
                'imidazole.sdf': 'Imidazole',
                'pyrazole.sdf': 'Pyrazole',
                'oxazole.sdf': 'Oxazole',
                'isoxazole.sdf': 'Isoxazole',
                'thiazole.sdf': 'Thiazole',
                'furan.sdf': 'Furan'
            }
