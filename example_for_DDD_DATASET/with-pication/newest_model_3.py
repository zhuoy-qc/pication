...
ARG_MODEL_PATH = "/data1/zyin/new_arg_model_results_benchmark/full_dihedral_model_vina_DDD/xgboost_model_optimized.pkl"         
# =====================================================================
...
def engineer_arg_features(df):
    ...
    # Create the exact features that the model was trained with
    ...
    return df[[
        'delta_z', 'distance', 'dihedral_angle', 'inv_distance', 
        'cos_dihedral', 'delta_z_norm', 'delta_x_norm', 
        'distance_delta_x', 'distance_delta_z', 'delta_x_delta_z'
    ]]
def run_model_prediction(temp_csv):
    ...
    if isinstance(model_data_arg, dict):
        ...
        scaler_arg = model_data_arg['scaler']
        selected_features_arg = model_data_arg['selected_features']
        all_feature_cols_arg = model_data_arg['all_feature_cols']
        logger.info(f"✅ Loaded ARG model (from dict) from: {ARG_MODEL_PATH}")
        logger.info(f"   Selected features: {selected_features_arg}")
    else:
    ...
    if not df_arg.empty:
        # Create a copy for feature engineering without modifying original data
        df_features = df_arg.copy()
        
        # Map input column names to training column names
        df_features = df_features.rename(columns={'Distance': 'distance', 'RZ': 'delta_z', 'Offset': 'delta_x', 'Dihedral_Angle': 'dihedral_angle'})
        
        # Apply same feature engineering as training
        df_features['inv_distance'] = 1 / df_features['distance']
        df_features['distance_sq'] = df_features['distance'] ** 2
        df_features['sin_dihedral'] = np.sin(np.radians(df_features['dihedral_angle']))
        df_features['cos_dihedral'] = np.cos(np.radians(df_features['dihedral_angle']))
        df_features['tan_dihedral'] = np.tan(np.radians(df_features['dihedral_angle']))
        df_features['delta_z_norm'] = df_features['delta_z'] / df_features['distance']
        df_features['delta_x_norm'] = df_features['delta_x'] / df_features['distance']
        df_features['delta_x_delta_z_ratio'] = df_features['delta_x'] / df_features['delta_z']
        df_features['distance_delta_z_ratio'] = df_features['distance'] / df_features['delta_z']
        df_features['distance_delta_x_ratio'] = df_features['distance'] / df_features['delta_x']
        df_features['log_distance'] = np.log(df_features['distance'])
        df_features['log_abs_delta_x'] = np.log(np.abs(df_features['delta_x']) + 1e-8)
        df_features['log_abs_delta_z'] = np.log(np.abs(df_features['delta_z']) + 1e-8)
        df_features['distance_cubed'] = df_features['distance'] ** 3
        df_features['delta_x_squared'] = df_features['delta_x'] ** 2
        df_features['delta_z_squared'] = df_features['delta_z'] ** 2
        df_features['distance_delta_x'] = df_features['distance'] * df_features['delta_x']
        df_features['distance_delta_z'] = df_features['distance'] * df_features['delta_z']
        df_features['delta_x_delta_z'] = df_features['delta_x'] * df_features['delta_z']
        df_features['effective_distance'] = np.sqrt(df_features['distance']**2 + df_features['delta_x']**2 + df_features['delta_z']**2)
        df_features['planar_distance'] = np.sqrt(df_features['delta_x']**2 + df_features['delta_z']**2)
        ...
        missing_cols = set(all_feature_cols_arg) - set(df_features.columns)
        if missing_cols:
            logger.error(f"❌ Missing required columns: {missing_cols}")
            sys.exit(1)

        # Select only the features used during training
        X = df_features[all_feature_cols_arg]

        # Check for any NaN values and report error if found
        invalid_rows = X.isnull().any(axis=1)
        if invalid_rows.any():
            logger.error(f"❌ Found {invalid_rows.sum()} rows with NaN values in features. Model expects no missing values.")
            problematic_rows = df_arg[invalid_rows].head()  # Show first few problematic rows
            logger.error(f"First problematic rows:\n{problematic_rows[['PDB_File', 'Protein', 'Distance', 'Offset', 'RZ', 'Dihedral_Angle']]}")
            sys.exit(1)
        ...
        if valid.any():
            ...
            X_scaled = scaler_arg.transform(X[valid])
            X_scaled_df = pd.DataFrame(X_scaled, columns=all_feature_cols_arg)

            # Select only the features the model was trained on
            X_selected = X_scaled_df[selected_features_arg]

            # Make predictions
            pred = model_arg.predict(X_selected)
            
            # Add predictions to the original dataframe (not the feature-engineered one)
    ...
    # Handle empty Dihedral_Angle - replace NaN with 'N/A'
    df_final['Dihedral_Angle'] = df_final['Dihedral_Angle'].fillna('N/A')
    
    # Round numeric columns (except for Dihedral_Angle which might now be 'N/A')
    num_cols = ['Distance','Offset','RZ','Angle','Adjusted_Angle','Predicted_Energy',
    ...
    output_columns = [
        'PDB_File', 'Binding_Site', 'Protein',
        'Distance', 'Offset', 'RZ', 'Dihedral_Angle', 'Ring_Center_X', 'Ring_Center_Y', 'Ring_Center_Z',
        'Ring_Normal_X', 'Ring_Normal_Y', 'Ring_Normal_Z', 'Interaction_Type', 
        'Predicted_Energy', 'PDB_ID', 'Energy_Rank'
    ]
