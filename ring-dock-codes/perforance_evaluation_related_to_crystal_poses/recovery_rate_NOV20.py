...
def calculate_metrics(report_file, predictions_file):
    ...
    # Create combined identifiers for matching PDB ID + Protein
    # For reference file, use Directory as PDB ID (it already contains the PDB ID part)
    df_report['PDB_Protein'] = df_report['Directory'].astype(str) + '_' + df_report['Protein'].astype(str)
    
    # For prediction file, use the existing PDB_ID column
    df_pred['PDB_Protein'] = df_pred['PDB_ID'].astype(str) + '_' + df_pred['Protein'].astype(str)

    # Calculate before filter metrics
    unique_report_all = set(df_report['PDB_Protein'].unique())
    unique_pred_all = set(df_pred['PDB_Protein'].unique())
    ...
    arg_report = set(df_report[df_report['ResidueType'] == 'ARG']['PDB_Protein'].unique())
    arg_pred = set(df_pred[df_pred['ResidueType'] == 'ARG']['PDB_Protein'].unique())
    ...
    lys_report = set(df_report[df_report['ResidueType'] == 'LYS']['PDB_Protein'].unique())
    lys_pred = set(df_pred[df_pred['ResidueType'] == 'LYS']['PDB_Protein'].unique())
    ...
    his_report = set(df_report[df_report['ResidueType'] == 'HIS']['PDB_Protein'].unique())
    his_pred = set(df_pred[df_pred['ResidueType'] == 'HIS']['PDB_Protein'].unique())
    ...
    # Recreate PDB_Protein identifier for filtered predictions
    df_pred_filtered['PDB_Protein'] = df_pred_filtered['PDB_ID'].astype(str) + '_' + df_pred_filtered['Protein'].astype(str)

    # Calculate after filter metrics
    unique_pred_filtered_all = set(df_pred_filtered['PDB_Protein'].unique())
    ...
    arg_filtered = set(df_pred_filtered[df_pred_filtered['ResidueType'] == 'ARG']['PDB_Protein'].unique())
    ...
    lys_filtered = set(df_pred_filtered[df_pred_filtered['ResidueType'] == 'LYS']['PDB_Protein'].unique())
    ...
    his_filtered = set(df_pred_filtered[df_pred_filtered['ResidueType'] == 'HIS']['PDB_Protein'].unique())
    ...
    arg_not_in_exp_before = len(set(df_pred[df_pred['ResidueType'] == 'ARG']['PDB_Protein'].unique()) & predictions_not_in_exp_before)
    arg_not_in_exp_after = len(set(df_pred_filtered[df_pred_filtered['ResidueType'] == 'ARG']['PDB_Protein'].unique()) & predictions_not_in_exp_after)
    ...
    lys_not_in_exp_before = len(set(df_pred[df_pred['ResidueType'] == 'LYS']['PDB_Protein'].unique()) & predictions_not_in_exp_before)
    lys_not_in_exp_after = len(set(df_pred_filtered[df_pred_filtered['ResidueType'] == 'LYS']['PDB_Protein'].unique()) & predictions_not_in_exp_after)
...
def main():
    ...
    # Save filtered predictions (excluding the temporary PDB_Protein column)
    df_filtered_to_save = results['df_pred_filtered'].drop(columns=['PDB_Protein'], errors='ignore')
    df_filtered_to_save.to_csv("remained_predictions.csv", index=False)
